...
- 自下而上/Top-down：适用于模型结构简单、结构独立、依赖少、后期迁移容易的场景。
- 自上而下/Bottom-up：适用于模型结构复杂、依赖多、后期迁移困难、需要结合尺寸强关联的零件。
...
- 启用`视图 菜单/用户界面/动态参考可视化(父级/子级)`，有助于观察零部件的参考情况。
...
- Feature manager design tree，`Roll back`功能：底部有返回前几步操作的拖动条，可以实现后期在某个已建特征之前插入新特征。
