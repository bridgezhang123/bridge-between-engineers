...
1. [命名标准](naming-standards.md)：先把文件名和目录结构统一起来。
    ...
    命名是整个系列的起点。若零件、装配体、图纸和标准件在一开始就缺乏统一规则，后续很容易出现装配引用混乱、同名文件冲突、图纸链接断裂等问题。因此，本系列先从零部件命名、标准件命名、外购件命名和目录结构讲起。
    ...
    这个结构不是为了把文件夹做复杂，而是为了让标准件、外购件、自制件、子装配体和版本迭代各有位置。对没有成熟 PDM 系统的个人或小团队来说，清晰的目录结构本身就是最基础的管理工具。

2. [建模方式](modeling-method.md)：再判断零件之间应采用怎样的建模关系。

    命名和目录明确之后，才适合进入具体建模。本文重点比较`自下而上/Bottom-up`与`自上而下/Top-down`两种思路：标准件、外购件和相对独立的零件适合自下而上；而端盖、壳体、螺钉过孔等存在强配合关系的结构，则更适合在装配体中建立必要的关联。
...
5. [图纸模板](drawing-template.md)：建立稳定的工程图表达框架。
    ...
    阅读这一章时，建议把重点放在“模板如何服务于复用”。模板不是为了排版好看，而是为了让每一张图纸都能稳定携带必要信息。
...
6. [基本标注](dimensioning.md)：让图纸真正可加工、可检验、可沟通。
    ...
    本文建议读者带着加工和检验视角阅读：这条尺寸是否必要、是否唯一、是否容易测量、是否会造成歧义。图纸越清楚，后续沟通成本越低。
