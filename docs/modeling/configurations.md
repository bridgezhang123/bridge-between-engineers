...
4. 关于爆炸视图：
    ...
    - 同一个装配体可以创建多个配置，每个配置可以拥有多个爆炸视图，以满足不同的需求。
    - 可以复制(Ctrl + c)配置下的某个爆炸视图而后粘贴(Ctrl + v)到其它配置，甚至是直接拖动到别的配置里，以此达到复用的效果。
    - 每个配置相互独立，例如隐藏某个配置对应模型的部分零部件，不影响其它配置的装配体。
    - 即使是子装配体内的零部件，也可以通过展开子装配体，选中特定零部件进行爆炸操作。
